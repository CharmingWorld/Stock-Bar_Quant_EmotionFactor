...
all_num = pd.DataFrame({'all_num_芒果超媒': all_num})
all_dic_score = pd.DataFrame({'all_dic_score': all_score})
weighted_mean_sentiment = pd.DataFrame({'芒果超媒舆情因子值': weighted_mean_score})
...
new = pd.concat([time, all_num, all_dic_score, weighted_mean_sentiment, positive_num, positive_sentiment, negative_num,
                 negative_sentiment],
                axis=1)

new['all_dic_score'] = new['all_dic_score'].apply(lambda x: format(x, '.2f'))
new['芒果超媒舆情因子值'] = new['芒果超媒舆情因子值'].apply(lambda x: format(x, '.2f'))
...
print(new)
new.to_csv('芒果超媒舆情因子数据(每日).csv', encoding='utf-8-sig', index=False)
final_result.to_csv('芒果超媒舆情因子数据(每条).csv', encoding='utf-8-sig', index=False)
