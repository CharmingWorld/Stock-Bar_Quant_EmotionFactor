...
all_num = pd.DataFrame({'all_num_迈瑞医疗': all_num})
all_dic_score = pd.DataFrame({'all_dic_score': all_score})
weighted_mean_sentiment = pd.DataFrame({'迈瑞医疗舆情因子值':  weighted_mean_score})
...
new['迈瑞医疗舆情因子值'] = new['迈瑞医疗舆情因子值'].apply(lambda x: format(x, '.2f'))
...
print(new)
new.to_csv('迈瑞医疗舆情因子数据(每日).csv', encoding='utf-8-sig', index=False)
final_result.to_csv('迈瑞医疗舆情因子数据(每条).csv', encoding='utf-8-sig', index=False)
